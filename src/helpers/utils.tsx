...
export function getDifficultyRussianName(difficulty: string): string {
  switch (difficulty) {
    case "mythic":
      return "Эпохальный";
    case "heroic":
...
export function getDungeonRussianName(dungeon: string): string {
  switch (dungeon) {
...
export function getTreasureFromLevel(level: number): number {
  if (level >= 20) {
    return 421;
  }
  switch (level) {
